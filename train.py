...
from model_v2 import SimpleGPT, SimpleGPTConfig
...
def load_checkpoint(
    ...
    try:
        ...
        return {
            ...
            "step": checkpoint["global_step"],  # Changed from "step" to "global_step"
...
def count_parameters(model):
    ...
    if len(model.layers) > 0:
        ...
        # Count feed-forward parameters
        ff_params = sum(
            p.numel() for p in model.layers[0]["ff"].parameters() if p.requires_grad
        )

        # Count layer norm parameters
        ln_params = sum(
            p.numel() for p in model.layers[0]["ln1"].parameters() if p.requires_grad
        ) + sum(
            p.numel() for p in model.layers[0]["ln2"].parameters() if p.requires_grad
        )

        # Total parameters per layer
        layer_params = attention_params + ff_params + ln_params

    total_layer_params = layer_params * len(model.layers)

    # Count head parameters
    main_head_params = model.main_head.weight.numel()

    second_head_params = sum(
        p.numel() for p in model.second_head.parameters() if p.requires_grad
    )
    ...
    return {
        ...
        "heads": {
            "total": main_head_params + second_head_params,
            "main_head": main_head_params,
            "second_head": second_head_params,
        },
    }
...
if __name__ == "__main__":
    ...
    #     expert_capacity_factor=1.5,  # Load balancing factor
    # )

    config = SimpleGPTConfig(
        vocab_size=1000,  # Update with your vocab size
        n_embd=256,
        n_head=4,
        n_layer=2,
        block_size=1024,
    ...
    # Enable static graph optimization if using DDP
    trainer.set_static_graph(True)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        metrics = trainer.train_epoch(
            ...
            save_every_n_steps=1000,  # Adjust as needed
